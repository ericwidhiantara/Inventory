...
class App extends Component {
    ...
    countTotalRefund = () => {
        let total = 0;
        for (let i = 0; i < Object.keys(refundTransaction).length; i++) {
            total += parseInt(refundTransaction[i].refundAmount, 10);
        }
        return total;
    }
    ...
    countAverageRefund = () => {
        ...
        return (total / Object.keys(refundTransaction).length);
    }
        ...
        return (
            <View style={{ flex: 1 }}>
                <DrawerLayout
                    ...
                    <Text style={{ textAlign: "center", marginTop: 10, fontSize: 22, fontWeight: 'bold' }}>REFUND REPORT</Text>
                    <Text style={{ marginLeft: 40, marginTop: 30, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Refund Count : </Text>{Object.keys(refundTransaction).length}</Text>
                    <Text style={{ marginLeft: 40, marginTop: 20, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Total Refund : </Text>{
                        
                        
                        MaskService.toMask('money', parseInt(this.countTotalRefund()), {
                        unit: 'Rp. ',
                        separator: ',',
                        delimiter: '.',
                        precision: 0,
                    ...
                    <Text style={{ marginLeft: 40, marginTop: 20, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Average Refund : </Text>
                    
                    {

                        MaskService.toMask('money', parseInt(this.countAverageRefund()), {
                        unit: 'Rp. ',
                        separator: ',',
                        delimiter: '.',
                        precision: 0,
