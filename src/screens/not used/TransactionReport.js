...
class App extends Component {
    ...
    countTotalSales = () => {
        let total = 0;
        for (let i = 0; i < Object.keys(shoppingTransaction).length; i++) {
            total += parseInt(shoppingTransaction[i].total, 10);
        }
        return total;
    }
    ...
    countAverageSales = () => {
        ...
        return (total / Object.keys(shoppingTransaction).length);
    }
        ...
        return (
            <View style={{ flex: 1 }}>
                <DrawerLayout
                    ...
                    <Text style={{ textAlign: "center", marginTop: 10, fontSize: 22, fontWeight: 'bold' }}>TRANSACTION REPORT</Text>
                    <Text style={{ marginLeft: 40, marginTop: 30, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Total Transaction : </Text>{Object.keys(shoppingTransaction).length}</Text>
                    <Text style={{ marginLeft: 40, marginTop: 20, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Total Sales : </Text>{
                        
                        
                        MaskService.toMask('money', parseInt(this.countTotalSales()), {
                        unit: 'Rp. ',
                        separator: ',',
                        delimiter: '.',
                        precision: 0,
                    ...
                    <Text style={{ marginLeft: 40, marginTop: 20, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Average Sales per Transaction : </Text>
                    ...
                    <Text style={{ marginLeft: 40, marginTop: 10, fontSize: 18 }}>

                        {
                            ...
                            MaskService.toMask('money', parseInt(this.countAverageSales()), {
                                unit: 'Rp. ',
                                separator: ',',
                                delimiter: '.',
                                precision: 0,
