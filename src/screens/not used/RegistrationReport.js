...
class App extends Component {
    ...
    countTotalTopUp = () => {
        let total = 0;
        for (let i = 0; i < Object.keys(registeredUser).length; i++) {
            total += parseInt(registeredUser[i].balance, 10);
        }
        return total;
    }
    ...
    countAverageTopUp = () => {
        ...
        return (total / Object.keys(registeredUser).length);
    }
        ...
        return (
            <View style={{ flex: 1 }}>
                <DrawerLayout
                    ...
                    <Text style={{ textAlign: "center", marginTop: 10, fontSize: 22, fontWeight: 'bold' }}>REGISTRATION REPORT</Text>
                    <Text style={{ marginLeft: 40, marginTop: 30, fontSize: 18 }}><Text style={{ fontWeight: 'bold' }}>Total Registered User : </Text>{Object.keys(registeredUser).length}</Text>
